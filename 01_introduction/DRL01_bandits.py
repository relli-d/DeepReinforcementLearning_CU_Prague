...
def main(env: MultiArmedBandits, args: argparse.Namespace) -> float:
    ...
    est_vals = np.full(args.bandits, args.initial)
    action_count = np.zeros(args.bandits)
    ...
    for step in range(args.episode_length):
        ...
        if args.alpha == 0:
                #Q(A) <- Q(A) + (R - Q(A)) / N(A)
            est_vals[action] += (reward - est_vals[action]) / action_count[action]
        else:
                #Q(A) <- Q(A) + alpha * (R - Q(A))
            est_vals[action] += args.alpha * (reward - est_vals[action])
