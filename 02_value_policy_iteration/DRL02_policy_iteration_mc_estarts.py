...
def main(args: argparse.Namespace) -> tuple[list[float] | np.ndarray, list[int] | np.ndarray]:
    ...
    value_function = None
    visit_counts = np.zeros((env.states, env.actions), dtype=np.int32)

    for _ in range(args.steps):
        #POLICY EVAL
        for start_state in range(env.states):
            for start_action in range(env.actions):
                state, action = start_state, start_action
                total_return, discount = 0.0, 1.0
                ...
                visit_counts[start_state, start_action] += 1
                alpha = 1 / visit_counts[start_state, start_action]
                action_value_function[start_state, start_action] += alpha * (total_return - action_value_function[start_state, start_action])
        
        #POLICY IMPROV
        for state in range(env.states):
            policy[state] = argmax_with_tolerance(action_value_function[state])

    value_function = np.max(action_value_function, axis=1)
