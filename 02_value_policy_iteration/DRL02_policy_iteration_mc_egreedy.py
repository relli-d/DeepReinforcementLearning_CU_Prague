...
def main(args: argparse.Namespace) -> tuple[list[float] | np.ndarray, list[int] | np.ndarray]:
    ...
    value_function = np.zeros(env.states)
    visit_counts = np.zeros((env.states, env.actions))
    ...
    for _ in range(args.steps):
        ...
        for i in range(env.states):
            ...
            power = 0
            state = i
            for j in range(args.mc_length):
                action = env.epsilon_greedy(args.epsilon, policy[state])
                if j == 0:
                    k = action
                reward, next_state = env.step(state, action)
                state = next_state
                returns += (args.gamma**(power))*reward
                power += 1
            visit_counts[i, k] += 1
            alpha = 1/visit_counts[i, k]
            action_value_function[i, k] += alpha * (returns - action_value_function[i, k])
