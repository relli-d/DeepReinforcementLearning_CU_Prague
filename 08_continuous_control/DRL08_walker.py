...
parser.add_argument("--target_entropy", default=-4, type=float, help="Target entropy per action component.")
...
class Agent:
    ...
    def __init__(self, env: npfl139.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        class Actor(torch.nn.Module):
            def __init__(self, hidden_size: int):
                ...
                self.mu_layer = torch.nn.Linear(hidden_size, act_dim)
                self.log_std_layer = torch.nn.Linear(hidden_size, act_dim)
                torch.nn.init.constant_(self.log_std_layer.bias, -1.0)

                # Scaling actions to environment range
                ...
                self.register_buffer("action_scale", ((action_high - action_low) / 2).to(Agent.device))
                self.register_buffer("action_offset", ((action_high + action_low) / 2).to(Agent.device))


                self.transform = torch.distributions.transforms.ComposeTransform([
                    torch.distributions.transforms.TanhTransform(),
                    torch.distributions.transforms.AffineTransform(
                        loc=self.action_offset, scale=self.action_scale)
                ], cache_size=1)

                # Learnable log alpha for entropy control
                self.log_alpha = torch.nn.Parameter(torch.tensor(np.log(0.1), dtype=torch.float32))

            def forward(self, obs: torch.Tensor, sample: bool):
        ...
        self.actor = Actor(args.hidden_layer_size).to(self.device)
        ...
        # Add scheduler (optional, use on one optimizer like actor for example)
    ...
    def train(self, states: torch.Tensor, actions: torch.Tensor, targets: torch.Tensor) -> None:
        sampled_actions, log_probs, alpha = self.actor(states, sample=True)
        q1 = self.critic1(states, sampled_actions)
        q2 = self.critic2(states, sampled_actions)
    ...
    def predict_values(self, next_states: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
...
def main(env: npfl139.EvaluationEnv, args: argparse.Namespace) -> None:
    ...
    while training:
        for _ in range(args.evaluate_each):
            ...
            #reward[done] = 0.0  # Override terminal penalties to zero to accelerate learning
        ...
        agent.step_scheduler(avg_return)
         ...
         # Save model if it's the first time the average return exceeds 0
        # or if we achieved a new best performance
