...
def main(args: argparse.Namespace) -> tuple[list[float] | np.ndarray, list[int] | np.ndarray]:
    ...
    # TODO: The final value function should be in `value_function` and final greedy policy in `policy`.
        #We do STEPS iterations of our evaluation+improvement routine
    for _ in range(args.steps):
        # POLICY EVALUATION: Solve (I - gamma * P) v = R
        ...
        for s in range(GridWorld.states):
            PRS = GridWorld.step(s, policy[s])
            for prob, reward, next_state in PRS:
                A[s, next_state] -= args.gamma * prob
                b[s] += prob * reward
        ...
        for s in range(GridWorld.states):
            ...
            for a in range(GridWorld.actions):
                PRS = GridWorld.step(s, a)
                action_values[a] = sum(prob * (reward + args.gamma * value_function[next_state]) for prob, reward, next_state in PRS)
            
            policy[s] = argmax_with_tolerance(action_values)
